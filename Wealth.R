...
# --- REMOVED: White Head of Household filter ---
# This section has been removed - no longer filtering to only white-headed households
...
# --- Remove households with top-coded individuals ---
...
# Filter to adults (age 18+) for individual analysis (note: men/women will be further filtered to white only)
...
# Split by gender (SEX=1 for men, SEX=2 for women) - apply RACE == 1 filter to men and women
men_18 <- ipums_1860_adults_no_topcoded[SEX == 1 & RACE == 1]
women_18 <- ipums_1860_adults_no_topcoded[SEX == 2 & RACE == 1]

cat(sprintf("  - Applied race filter to men: %s white men remain\n", 
            format(nrow(men_18), big.mark = ",")))
cat(sprintf("  - Applied race filter to women: %s white women remain\n", 
            format(nrow(women_18), big.mark = ",")))

# Create white heads dataset for Tables 5-7 Panel (c)
white_heads <- ipums_1860_adults_no_topcoded[RELATE == 1 & RACE == 1]
cat(sprintf("  - Identified %s white household heads\n", 
            format(nrow(white_heads), big.mark = ",")))
...
# Calculate wealth statistics by region for men, women, and households (men/women are white)
...
# Run occupational analysis for North/South men, women, and white heads
...
north_heads_results_occ <- run_occupational_analysis(white_heads[MACRO_REGION == "North"], "North Heads")
south_heads_results_occ <- run_occupational_analysis(white_heads[MACRO_REGION == "South"], "South Heads")
...
# Create main wealth statistics tables (white men/women, all households)
...
# Create tables comparing North/South by occupation with consistent ordering (now with 3 panels)
create_merged_occ_table_3_panels <- function(north_res, south_res, north_heads_res, south_heads_res, value_col) {
  ...
  north_heads_data <- prepare_occ_data_for_table(north_heads_res, value_col, "North_Heads")
  south_heads_data <- prepare_occ_data_for_table(south_heads_res, value_col, "South_Heads")
  
  # Create base table with all occupations in correct order
  base_occ_dt <- data.table(occupation = occ_order, order_value = 1:length(occ_order))
  
  # Merge all data, preserving all occupations
  ...
  merged <- merge(merged, north_heads_data, by = c("occupation", "order_value"), all.x = TRUE)
  merged <- merge(merged, south_heads_data, by = c("occupation", "order_value"), all.x = TRUE)
...
# Create occupation tables with 3 panels (a: White Men, b: White Women, c: White Heads)
table5_men <- create_merged_occ_table_3_panels(north_men_results_occ, south_men_results_occ, 
                                               north_heads_results_occ, south_heads_results_occ, "percentage")
table5_women <- create_merged_occ_table_3_panels(north_women_results_occ, south_women_results_occ, 
                                                 north_heads_results_occ, south_heads_results_occ, "percentage")
table6_men <- create_merged_occ_table_3_panels(north_men_results_occ, south_men_results_occ, 
                                               north_heads_results_occ, south_heads_results_occ, "mean_wealth")
table6_women <- create_merged_occ_table_3_panels(north_women_results_occ, south_women_results_occ, 
                                                 north_heads_results_occ, south_heads_results_occ, "mean_wealth")
table7_men <- create_merged_occ_table_3_panels(north_men_results_occ, south_men_results_occ, 
                                               north_heads_results_occ, south_heads_results_occ, "median_wealth")
table7_women <- create_merged_occ_table_3_panels(north_women_results_occ, south_women_results_occ, 
                                                 north_heads_results_occ, south_heads_results_occ, "median_wealth")

# Rename columns for the heads panels
for (tbl in list(table5_men, table5_women, table6_men, table6_women, table7_men, table7_women)) {
  if ("North_Heads" %in% colnames(tbl)) {
    colnames(tbl)[colnames(tbl) == "North_Heads"] <- "North_Heads"
  }
  if ("South_Heads" %in% colnames(tbl)) {
    colnames(tbl)[colnames(tbl) == "South_Heads"] <- "South_Heads"
  }
}
...
# Calculate county-level statistics for women
county_female_stats <- women_18[, .(
  Mean_women = weighted.mean(WEALTH_TOTAL, PERWT, na.rm = TRUE),
  Median_women = weighted_quantile(WEALTH_TOTAL, 0.5, weights = PERWT),
  Gini_women = gini(WEALTH_TOTAL, PERWT),
  observations_women = .N
), by = .(GISJOIN)]

# Calculate county-level occupation percentages for white men
county_male_occ_stats <- men_18[, {
  total_men = .N
  farmers_planters = sum(OCC1950 %in% c(100, 123), na.rm = TRUE)
  # Unskilled laborers: non-family farm laborers + non-farm laborers
  non_family_farm_laborers = sum(OCC1950 %in% c(810, 820, 830, 840) & !(RELATE >= 2 & RELATE <= 10), na.rm = TRUE)
  non_farm_laborers = sum(OCC1950 >= 910 & OCC1950 <= 970, na.rm = TRUE)
  unskilled_laborers = non_family_farm_laborers + non_farm_laborers
  
  .(
    Pct_men_farmers_planters = ifelse(total_men > 0, (farmers_planters / total_men) * 100, 0),
    Pct_men_unskilled_laborers = ifelse(total_men > 0, (unskilled_laborers / total_men) * 100, 0)
  )
}, by = .(GISJOIN)]

# Calculate county-level occupation percentages for white women
county_female_occ_stats <- women_18[, {
  total_women = .N
  farmers_planters = sum(OCC1950 %in% c(100, 123), na.rm = TRUE)
  # Unskilled laborers: non-family farm laborers + non-farm laborers
  non_family_farm_laborers = sum(OCC1950 %in% c(810, 820, 830, 840) & !(RELATE >= 2 & RELATE <= 10), na.rm = TRUE)
  ...
  .(
    Pct_women_farmers_planters = ifelse(total_women > 0, (farmers_planters / total_women) * 100, 0),
    Pct_women_unskilled_laborers = ifelse(total_women > 0, (unskilled_laborers / total_women) * 100, 0)
  )
}, by = .(GISJOIN)]

# Calculate county-level occupation percentages for white heads
county_heads_occ_stats <- white_heads[, {
  total_heads = .N
  farmers_planters = sum(OCC1950 %in% c(100, 123), na.rm = TRUE)
  # Unskilled laborers: non-family farm laborers + non-farm laborers
  non_family_farm_laborers = sum(OCC1950 %in% c(810, 820, 830, 840) & !(RELATE >= 2 & RELATE <= 10), na.rm = TRUE)
  ...
  .(
    Pct_heads_farmers_planters = ifelse(total_heads > 0, (farmers_planters / total_heads) * 100, 0),
    Pct_heads_unskilled_laborers = ifelse(total_heads > 0, (unskilled_laborers / total_heads) * 100, 0)
  )
}, by = .(GISJOIN)]

# Combine county statistics
county_stats_combined <- merge(county_household_stats, county_male_stats, by = "GISJOIN", all = TRUE)
county_stats_combined <- merge(county_stats_combined, county_female_stats, by = "GISJOIN", all = TRUE)
county_stats_combined <- merge(county_stats_combined, county_male_occ_stats, by = "GISJOIN", all = TRUE)
county_stats_combined <- merge(county_stats_combined, county_female_occ_stats, by = "GISJOIN", all = TRUE)
county_stats_combined <- merge(county_stats_combined, county_heads_occ_stats, by = "GISJOIN", all = TRUE)
...
output_file_path <- file.path(output_dir, "Wealth.txt")
...
# Define tables to print with updated titles reflecting white men/women but all households
tables_to_print <- list(
  list(title = "TABLE 1: Mean Wealth, 1860 (White Men/Women, All Households, Age 18+)", table = table1, digits = 0),
  list(title = "TABLE 2: Median Wealth, 1860 (White Men/Women, All Households, Age 18+)", table = table2, digits = 0),
  list(title = "TABLE 3: Gini Coefficients for Wealth, 1860 (White Men/Women, All Households, Age 18+)", table = table3, digits = 3),
  ...
  list(title = "TABLE A.4: Household Wealth Statistics by Region (South disaggregated, All Households)", table = table_A4_data_fixed, digits = c(0,0,0,3)), 
  list(title = "TABLE A.5a: Household Wealth Statistics (Farm Household Heads, All Households)", table = table_A5a_data_fixed, digits = c(0,0,0,3)), 
  list(title = "TABLE A.5b: Household Wealth Statistics (Non-Farm Household Heads, All Households)", table = table_A5b_data_fixed, digits = c(0,0,0,3)) 
)
...
for (item in tables_to_print) {
  ...
  if (is.null(item$table) || nrow(item$table) == 0) {
    ...
    } else if (grepl("TABLE [567][ab]:", item$title)) {
      # For occupation tables with 3 panels, rename the heads columns
      if ("North_Heads" %in% colnames(current_table_df)) {
        colnames(current_table_df)[colnames(current_table_df) == "North_Heads"] <- "North (Heads)"
      }
      if ("South_Heads" %in% colnames(current_table_df)) {
        colnames(current_table_df)[colnames(current_table_df) == "South_Heads"] <- "South (Heads)"
      }
    }
    ...
    if (item$title %in% c("TABLE A.4: Household Wealth Statistics by Region (South disaggregated, All Households)",
                          "TABLE A.5a: Household Wealth Statistics (Farm Household Heads, All Households)",
                          "TABLE A.5b: Household Wealth Statistics (Non-Farm Household Heads, All Households)")) {
    ...
    } else if (grepl("TABLE [567][ab]:", item$title) && ncol(current_table_df) == 5) {
      # For 3-panel occupation tables (occupation + 4 data columns)
      kable_digits <- c(0, rep(item$digits[1], 4))
    }
